...
class Post(models.Model):
    ...
    tags = models.ManyToManyField('Tag', blank = True) # 'Tag' 모델 클래스의 이름을 문자열로 하는 이유는 장고가 나중에 처리하도록 하기 위해서. 문자열이 아닐 경우 Post 클래스보다 앞에 있어야 함.
    category = models.ForeignKey(Category, null = True, blank = False) # null은 db에서 사용하는 것 blank는 폼에서 사용하는 것
